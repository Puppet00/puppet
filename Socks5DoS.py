...
def clone(): Socks5 List
	r = requests.get('https://api.proxyscrape.com/?request=displayproxies&proxytype=socks5&country=all&anonymity=all&timeout=99999') #Code By GogoZin
...
def main():
	...
	global per
	global uu
	url = str(input('[+] Target ( www.google.com ) : '))
	if url =='':
	...
	if thr =='':
		...
		thr = int(thr)
	per = str(input("[+] CC-Power ( Default 70 ) : "))
	if per =='':
		per = int(70)
	else:
		per = int(per)
